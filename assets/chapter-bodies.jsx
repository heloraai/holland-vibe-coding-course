  ...
  document.getElementById('greet')
    .addEventListener('click', () => {
      document.getElementById('msg')
        ...
        <section id="d">
          <h2>D · 几个真实做出来的东西</h2>
          <p>说再多不如看例子。下面这些都是一个人用 Vibe Coding 方式做出来的，作者大多不是专业程序员：</p>
          <CardList items={[
            { t: 'SleepForest · 本课主例', d: '4 个场景 + 4 只小动物 + 4-7-8 呼吸法 + AI 实时独白。需求一句话："睡不着，想有人陪着呼吸。"' },
...
...`}</CodeBlock>
  ...
  8: {
    ...
    sections: [
      { id: 'a', label: '你现在掌握了什么' },
      { id: 'b', label: '接下来 3 个月的路线' },
      { id: 'c', label: '什么时候该学"真"编程' },
      { id: 'd', label: '常去的地方' },
      { id: 'e', label: '最后几句话' },
    ],
    Body: () => (
      <>
        <section id="a">
          <h2>A · 你现在掌握了什么</h2>
          <p>课程走完了。盘一下你现在的能力地图：</p>
          <CardList items={[
            { n: '✓', t: '把一个想法讲清楚', d: '1-Pager + 一句话 Concept。' },
            { n: '✓', t: '把想法变成可点的原型', d: 'AI Studio + Prompt 迭代。' },
            { n: '✓', t: '让 AI 在真项目里改代码', d: 'Codex + Plan/Apply 节奏。' },
            { n: '✓', t: '理解一个前端项目的骨架', d: '组件 / 状态 / 副作用。' },
            { n: '✓', t: '调试和审美打磨', d: 'what + want 的语言 + 5 个细节。' },
            { n: '✓', t: '把东西发布到互联网', d: 'GitHub + Vercel + 环境变量。' },
          ]} />
          <Callout kind="tip">
            这套能力不是"入门"——是<strong>一个完整的闭环</strong>。你现在能独立走完"想法 → 上线 → 反馈"这条线。很多所谓的"做产品的人"也做不到这件事。
          </Callout>
        </section>
        ...
        <section id="b">
          <h2>B · 接下来 3 个月的路线</h2>
          <CardList items={[
            { n: '月 1', t: '再做 2 个小项目', d: '别做大的。每个 1-2 周。每次目标就是"再熟一遍 8 Stage"。可以参考 Pieter Levels 的 "12 startups in 12 months"——核心不是真做出 12 个赚钱产品，而是逼自己形成"从零跑完一遍"的肌肉记忆。他后来的 PhotoAI、Remote OK 都是在这个节奏里长出来的。' },
            { n: '月 2', t: '找一个在用你东西的人', d: '不是朋友为了支持你——是真的在用。哪怕只有 1 个。然后为他做迭代。小猫补光灯上线第一周的迭代全部来自即刻上陌生人的真实反馈："猫眨眼太快"、"长按能不能调色温"、"前置摄像头也要"，不是作者自己的脑补。' },
            { n: '月 3', t: '做一个稍大的项目', d: '比如加入数据库、登录、多页面。这时候开始学"真"编程也不迟。可以参考 Cal AI 的路径——从"一键拍照算卡路里"这个最小核心，半年内长成带订阅、账户、历史记录、社群的完整 SaaS。不是一开始就贪多，是在小核心真被人用之后才加的。' },
          ]} />
        </section>
        ...
        <section id="c">
          ...
          <p>当你反复遇到这几类情况：</p>
          <Compare
            left={<>你给 Codex 描述<strong>需要很长段</strong>才能讲清一个小改动。</>}
            right={<>你看懂了大部分代码，只是<strong>还不能自己写</strong>。</>}
            leftLabel="信号"
            rightLabel="反向的信号"
          />
          <p>前者意味着你需要多懂一点语言机制，才能讲得更精确；后者意味着你已经自然长出了直觉。两种都是好状态——都是进阶的入口。</p>
          <Callout kind="note" title="从哪开始">
            JavaScript + React 的基础教程就行。<strong>不要</strong>去系统地学"计算机科学"——那是给做底层工程师的路径，跟你要做的事不是一回事。</Callout>
        </section>

        <section id="d">
          <h2>D · 常去的地方</h2>
          <CardList items={[
            { t: '即刻 · #AI编程 / #独立开发', d: '中文独立开发者最集中的地方。小猫补光灯、各路周末项目都先在这里被人看到、被人用。进去别只潜水——发一条"我做了这个"，你会发现这社区比你想象的友好。' },
            { t: 'X · #buildinpublic', d: 'Pieter Levels、Marc Lou、@jasonzhou1993 这些一人公司的 founder 天天在这个 tag 下分享"今天做了啥、今天赚了多少"。跟 5 个你喜欢的 builder，刷一个月，你对"一个人能做到什么程度"的认知会完全更新。' },
            { t: 'Show HN / Product Hunt', d: '看别人做的小东西。学"什么是好的 one-person app"。fly.pieter.com、Cal AI 都是在这两个地方第一次被大范围看见的。' },
            { t: 'Are.na / Pinterest', d: '纯视觉灵感库。逼自己每周存 10 张。' },
            { t: 'Dribbble 但只看 App Interaction', d: '动效和微交互，是品味训练场。' },
            { t: 'GitHub Trending', d: '每周看 10 分钟。看趋势，不必每个都用。' },
          ]} />
        </section>

        <section id="e">
          <h2>E · 最后几句话</h2>
          <p>这门课不教你"学会编程"。它教你<strong>"会做东西"</strong>——这是两件事。后者比前者稀有。</p>
          <p>做一个真能跑的东西，比做 100 个"理论上能 work"的 demo 都重要。</p>
          <blockquote>
            "品味比语法重要。"<br />
            这句话从 Ch 1 到现在出现了不下 10 次。你现在应该懂它在说什么。
          </blockquote>
    ...
    homework: [
      '写一段话（200 字左右）：这门课之前 vs 之后，你对"做东西"的理解变了什么',
      '列出接下来 3 个月想做的 3 个小项目的一句话',
      '给自己一个"写完课程总结"的奖励',
    ],
  },
};
