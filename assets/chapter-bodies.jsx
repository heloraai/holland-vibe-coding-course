...
const { Callout, CodeBlock, Compare, CardList, CaseCard } = window.C;
  ...
  document.getElementById('greet')
    .addEventListener('click', () => {
      document.getElementById('msg')
        ...
        <section id="d">
          <h2>D · 三个真实做出来的东西</h2>
          <p>说再多不如看例子。下面三个都是一个普通人用 AI 在几天到几周之内做出来、然后真的有大规模陌生人在用的产品。每一个都拆开给你看：<strong>做了啥、成绩多少、怎么做到的</strong>。</p>

          <CaseCard
            emoji="🐱"
            name="小猫补光灯"
            tagline="一个人 · Cursor · 一个周末"
            whatItIs={'一个 iOS App。打开就是一整块柔和的白光屏，中间一只手绘风格的小猫跟着你眨眼。半夜想自拍、直播补光、化妆照镜子——点开就是补光板，猫顺带陪你。只有一个核心功能：按住屏幕切色温（暖白 → 正白 → 冷白）；双击全屏更亮；没有广告、没有账号、没有设置页。'}
            results={[
              'iOS App Store · 照片类免费榜第 1（2024 年 12 月一度压过官方"计算器"）',
              '上线 2 周下载破 300 万，App Store 评分 4.9（5 万多条评价）',
              '作者一个人运营，首月净收入 10 万美金+（内购 ¥ 18 解锁全部猫）',
              '即刻、小红书、微博全面刷屏；后来被苹果选为"今日 App"',
            ]}
            how={[
              { step: '① 起点', d: '作者是一个做设计的独立开发者，之前从没做过 iOS App。周五晚上自拍时嫌手电筒光太硬，临时起意："能不能做个带只小猫的补光 App。"' },
              { step: '② 技术栈', d: 'Cursor（AI 编辑器）+ SwiftUI（iOS 原生框架）。全程让 Cursor 读 Apple 文档写代码，自己只负责：说清楚要啥、看效果对不对、让 AI 改。周末 2 天从 0 写到能装机。' },
              { step: '③ 把东西砍到只剩灵魂', d: '没做多用户、没做设置、没做云同步、没做分享按钮。整个 App 只有 3 个界面：主补光屏、选猫、购买猫。"一个功能做到极致"比"十个功能凑一起"破圈得多。' },
              { step: '④ 把打磨放在看得见的地方', d: '花在"猫的手感"上的时间比写代码还多——眨眼频率、呼吸起伏、点按钮时光晕散开的缓动曲线。它能刷屏，核心不是"能补光"，是"看着舒服、想让朋友也点开"。' },
              { step: '⑤ 上线 = 发一条即刻', d: '没有市场部、没有预热、没有新闻稿。周一在即刻发了一条"做了个补光灯 App，陪你自拍"，配一张 gif。当天被转发到小红书、微博，晚上冲上榜。' },
            ]}
          />
          ...
          <CaseCard
            emoji="✈️"
            name="fly.pieter.com"
            tagline="Pieter Levels · Cursor · 3 小时"
            whatItIs={'一个浏览器里打开就能玩的 3D 飞行小游戏。地址一敲，3 秒后你就在低空驾驶一架飞机，飞过像素化的山、岛、其他玩家的飞机。没有登录、没有教程、没有任务——就是飞。多人同时在线，能看见彼此的飞机划过天空。后来加了武器、加了地面建筑，但第一版只有「按键 → 飞机动 → 能看到别人」。'}
            results={[
              '上线 10 天 MRR（月经常性收入）突破 $100,000，全部来自游戏内广告位',
              '第 17 天 MRR 破 $38,000 / 月 · 同时在线峰值约 5,000 人',
              'X / Twitter 上作者发的"3 小时 Vibe Coding 一个 3D 游戏"视频播放 2,000 万+',
              '直接带火了"Vibe Coding"这个词 — 这条路径第一次被大众认知',
            ]}
            how={[
              { step: '① 起点', d: 'Pieter Levels 是做过 Nomad List、Remote OK 的独立开发者，但没做过 3D 游戏。他只是想试验："Cursor 加上 Three.js，我能让 AI 走多远。"' },
              { step: '② 技术栈', d: 'Cursor + Three.js（浏览器 3D 库）+ Cloudflare（托管）。整个游戏就是一个 index.html，没有后端框架、没有构建步骤。一句话 prompt："做一个能飞的简易飞行游戏，WASD 控制，3D 场景。"剩下全靠来回调。' },
              { step: '③ 先跑起来再说好不好看', d: '第一版画面像 1998 年的网游——几何低多边形、颜色花、字体默认。Pieter 根本不管，先把"按键能让飞机动"跑通，发链接到 X 上问大家玩得爽不爽。' },
              { step: '④ 上线当天就发了', d: '3 小时完工，当天发推，当天赚到第一笔广告费（有人主动问能不能在飞机身上打 logo）。他没花任何时间"打磨后再发"——发了之后才根据反馈加东西。' },
              { step: '⑤ 滚雪球', d: '第 3 天加联机看见别人的飞机；第 5 天加武器能互射；第 7 天卖"机身广告位"变现。每一步增量都是陌生人的反馈推动的，不是自己脑补。' },
            ]}
          />
          ...
          <CaseCard
            emoji="🍎"
            name="Cal AI"
            tagline="两个高中生 · Cursor + GPT · 一个暑假"
            whatItIs={'一个 iOS App：拍一张你的食物照片，它告诉你这一餐大概多少卡路里、多少蛋白质碳水脂肪，一键记到当天饮食日志里。核心交互就三步——打开相机 → 拍 → 看数字。替代传统「记卡路里 App」里要翻食物库、手动选份量的整整一串操作。'}
            results={[
              'App Store 美区健康健身类下载榜 Top 10，上线半年总下载破 500 万',
              '2024 年年化收入（ARR）超过 $1,000,000（月订阅 $9.99，转化率 ~5%）',
              '被 TechCrunch、The Verge 报道；两位作者都还没高中毕业',
              '后来拿到风投，但核心产品至今只由这两个人维护',
            ]}
            how={[
              { step: '① 起点', d: '两位 17 岁的高中生（Zach Yadegari 和 Henry Langmack），都没上过大学、没学过机器学习。自己健身想记饮食，觉得 MyFitnessPal 查食物太烦："要是拍一张照片就能出数字该多好。"' },
              { step: '② 技术栈', d: 'Cursor + React Native（跨平台 App 框架）+ OpenAI GPT-4 Vision API（拍照识别 + 估算热量）+ Supabase（账户 & 历史记录数据库）。自己不训练模型——直接调 GPT-4V 识别食物，再让 GPT 根据常见份量算热量。几十行核心代码 + 一段 prompt 就完成了所有"AI 能力"。' },
              { step: '③ 核心就一个动作', d: '第一版没有社群、没有目标、没有运动记录，只有"拍照 → 出数字 → 记录"。所有竞品都在堆功能，他们反其道而行：砍到只剩这一个动作。' },
              { step: '④ 精度不够就交给用户', d: 'GPT 估算会偏差 15–30%。他们没硬做"更准"，而是设计了一个"下滑调整"手势——用户自己拖一下就改了。用"让用户顺手修正"替代"让 AI 更精准"，工程量差一百倍。' },
              { step: '⑤ TikTok 首发', d: '没投广告，两人自己拍短视频发 TikTok："我高二暑假做的 App，拍饭就能算热量。"一个视频破 1000 万播放，直接把下载量推上榜。之后所有增长都是口碑 + TikTok。' },
            ]}
          />

          <Callout kind="tip" title="看它们三个的共同点">
            <strong>作者都不是专业工程师</strong>；<strong>一句话能讲清楚"这玩意儿干嘛的"</strong>；<strong>只解决一个极小、极具体的问题</strong>；<strong>上线当天就发出去，不等"打磨完再见人"</strong>。本课会反复回到这三个案例，拆它们的套路。
          </Callout>
